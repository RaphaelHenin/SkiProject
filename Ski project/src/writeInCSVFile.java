...
public class writeInCSVFile {
	public static void write(ArrayList<Arc> arcs, String path) throws FileNotFoundException {
		...
		Double[][] adjacentMatrix = new Double[37][37];
		if (!csvFile.exists())
			throw new FileNotFoundException("Le fichier n'existe pas");
		else {
			PrintStream l_out = new PrintStream(new FileOutputStream(path, false));
			for (Arc arc : arcs) {
				adjacentMatrix[arc.getSource().getId()-1][arc.getDestination().getId()-1] = arc.getTime();
			}
